...
export default function WorkoutForm({ defaultType, templateId }: { defaultType?: string; templateId?: string }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const { data: profile } = await supabase.from('profiles').select('role, preferred_weight_unit').eq('id', user.id).single()
                    ...
                    <div style={{ marginBottom: '0.5rem' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem' }}>
                        {(['sets', 'reps', 'weight'] as const).map((f) => (
                          <div key={f}>
                            <label style={{ ...labelBase, whiteSpace: 'nowrap' }}>{f === 'weight' ? `Weight (${preferredWeightUnit})` : f.charAt(0).toUpperCase() + f.slice(1)}</label>
