...
Recent Changes:
	...
	•	Waitlist/no-show/payment tracking added for both bball_classes (bball_class_signups.status: booked/waitlist/no_show) and scheduled_classes (class_attendees.status gains 'waitlist'; 'absent' now doubles as its no-show state; scheduled_classes gains nullable max_slots), both signup tables gain payment_status (unpaid/paid_online/paid_cash) + amount_paid; capacity triggers (check_bball_class_capacity, new check_scheduled_class_capacity) now auto-waitlist on insert instead of rejecting — see supabase_class_waitlist_payment_tracking.sql. Members never see other attendees' names/payment info, see "{spots left}" instead of a headcount, and see their own payment/waitlist status; joining a full class now succeeds onto the waitlist (lib/utils.ts's joinBballClass() centralizes the bball insert+status readback, used by app/classes/page.tsx, app/calendar/page.tsx, app/coach/page.tsx, and components/BballClassModal.tsx). Admin/coach clicking any class (bball or scheduled) now navigates to a dedicated roster page (components/ClassRosterView.tsx, routed at app/classes/[id]/page.tsx and app/calendar/classes/[id]/page.tsx via a ?date= query param) instead of opening a modal — Booked/Waitlist/No Show tabs with search, sort, payment-status filter, and amount_paid min/max filter, plus an inline per-row editor (payment status, amount paid, mark no-show/move to booked) and existing add/remove-attendee reuse.