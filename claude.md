...
Recent Changes:
	...
	•	components/AthleteProgramTable.tsx's per-day exercise table dropped the day-of-week prefix from its accordion header (now shows just the day's title, e.g. "Test" instead of "Sunday — Test") and removed the per-exercise TYPE column/select entirely (the Type search filter still exists and still filters on exercise_type under the hood, it's just no longer an editable/visible column). Each exercise row now has a per-exercise Simple/Detailed toggle pill: Simple keeps the flat Sets/Reps/Weight number inputs; Detailed replaces those three cells with a per-set editor (add/remove individual "Set N: reps @ weight" rows, stored in the new athlete_program_exercises.set_details jsonb column — see supabase_program_exercise_set_details.sql) while sets/reps/weight keep auto-updating to a computed summary (count of sets, and the max-weight set's reps/weight) on every add/remove/blur so Simple-mode views and existing summary consumers stay correct without a migration.
