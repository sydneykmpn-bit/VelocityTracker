...
Additional feature tables (ask before assuming exact columns):
	...
	•	athlete_programs (id, coach_id, member_id, created_at) + athlete_program_days (id, program_id, day_of_week[0=Sun..6=Sat], title, created_at) + athlete_program_exercises (id, program_day_id, name, exercise_type, sets, reps, weight, notes, set_details jsonb, created_at) — one program_day row per (program, weekday) holding that day's title (preset: Upper/Legs/Shoulders/Sprint-Power, or custom), with its exercises nested under program_day_id (day_of_week now lives on the parent day, not the exercise). set_details is nullable, populated only when a coach puts an exercise into "Detailed" mode (per-set reps/weight breakdown, toggled per-exercise in components/AthleteProgramTable.tsx); sets/reps/weight always stay populated as a computed summary (max-weight set) even in Detailed mode, mirroring the same convention as the top-level exercises table's Advanced Mode. Deleting an athlete_programs row cascades its days, which cascade their exercises. athlete_program_completions (id, program_day_id, occurrence_date, completed_at, auto_logged_workout_id) tracks per-week "Mark Done" state on the athlete side — one row per (program_day_id, occurrence_date), where occurrence_date is that weekday's date within the current calendar week (lib/utils.ts's getCurrentWeekOccurrenceDate()); marking done auto-logs a real workouts+exercises row (same auto-log-from-plan shape used elsewhere) and links it via auto_logged_workout_id, "Undo" deletes that workout+exercises+completion row. Shared UI in components/AthleteProgramTable.tsx renders days as a day-grouped accordion (coach: editable day titles/exercises + Add Day/Add Exercise; student: read-only + Mark Done/Undo), both with a Search + Type filter (Type filter is exercise_type, a field no longer surfaced as an editable column — see Recent Changes). Superseded the old programs/program_workouts/program_workout_exercises/program_assignments model — those tables still exist in the DB but nothing reads/writes them anymore
...
Recent Changes:
	...
	•	athlete_programs gains status ('pending'|'completed', default 'pending') + completed_at (see supabase_athlete_programs_status.sql), tracking the whole assigned program as done vs in-progress — distinct from athlete_program_completions' per-day-per-week tracking. Student's Programs tab (app/student/page.tsx) gets Pending/Done sub-tabs filtering assignedPrograms by status, plus a "Mark Program Done"/"Mark Pending" button per program card; coach's Programs tab (app/coach/page.tsx) shows the same status as a read-only pill (list row + detail header) — status is member-driven only, coaches don't toggle it, matching the workout_plans.status convention where completion is a member action.
	...
	•	athlete_programs gains auto_logged_workout_id (uuid references workouts(id) on delete set null) — student's "Mark Program Done" (app/student/page.tsx) now auto-logs a real workouts+exercises row (title = coach's program name, exercises flattened from every day in the program), same auto-log-from-plan shape used elsewhere, and "Mark Pending" undoes it by deleting that workout+exercises, mirroring the per-day Mark Done/Undo pattern.
	•	Member dashboard (app/dashboard/page.tsx) shows a "You have programs for the week" banner (teal, matches the admin/coach banner styling) when the logged-in member has at least one athlete_programs row with status='pending'; links to /student?tab=programs. app/student/page.tsx now reads an initial `?tab=` query param (wrapped in a Suspense boundary for useSearchParams) to land directly on a given tab.
